...
const rawCrops: CropConfig[] = [
  ...
  { name: '黄瓜', priceCoefficient: 215.0457, maxWeight: 13.60, growthSpeed: 50, type: '普通', specialMutations: ['黄瓜蛇'] },
  ...
  { name: '草莓', priceCoefficient: 8706.3654, maxWeight: 8.50, growthSpeed: 888, type: '普通', specialMutations: ['连体'] },
  ...
  { name: '松果', priceCoefficient: 4615.5964, maxWeight: 17.00, growthSpeed: 570, type: '普通' },
  ...
  { name: '惊奇菇', priceCoefficient: 49680.1819, maxWeight: 10.20, growthSpeed: 2400, type: '普通' },
  ...
  // 月球作物
  { name: '月光草', priceCoefficient: 343.8951, maxWeight: 3.40, growthSpeed: 111, type: '月球' },
  { name: '灰壤豆', priceCoefficient: 1374.6159, maxWeight: 3.40, growthSpeed: 180, type: '月球' },
  { name: '月灯草', priceCoefficient: 1498.0086, maxWeight: 5.10, growthSpeed: 205, type: '月球' },
  { name: '月番茄', priceCoefficient: 662.4713, maxWeight: 10.20, growthSpeed: 111, type: '月球' },
  { name: '月环树', priceCoefficient: 322.0468, maxWeight: 20.40, growthSpeed: 58, type: '月球' },
  ...
  { name: '月核树', priceCoefficient: 3388.6303, maxWeight: 17, growthSpeed: 460, type: '月球' },
  { name: '液光藤', priceCoefficient: 3512.6104, maxWeight: 20.4, growthSpeed: 520, type: '月球' },
  { name: '幻月花', priceCoefficient: 8362.3108, maxWeight: 23.8, growthSpeed: 1142.5, type: '月球' },
  { name: '星空玫瑰', priceCoefficient: 33096.7468, maxWeight: 10.2, growthSpeed: 2966.6, type: '月球' },
  { name: '月兔', priceCoefficient: 11161.3085, maxWeight: 23.8, growthSpeed: 1524, type: '月球' },
  { name: '红包树', priceCoefficient: 83300.1019, maxWeight: 5.10, growthSpeed: 5270, type: '月球' },
  { name: '月影梅', priceCoefficient: 77270.106, maxWeight: 3.40, growthSpeed: 4000, type: '月球' },
]
...
/** 作物名称别名映射：果实名 → 种子/树名（用于兼容旧数据与图片命名） */
...
export const getCropByName = (name: string): CropConfig | undefined => {
  const canonicalName = CROP_NAME_ALIASES[name] ?? name
  return crops.find(crop => crop.name === canonicalName)
}

/**
 * 获取在界面上展示用的作物名称（树名 → 果实名）
 */
export const getCropDisplayName = (name: string): string => {
  switch (name) {
    case '月核树':
      return '月核果'
    case '月环树':
...
export const getCropImagePath = (cropName: string): string => {
  const canonicalName = CROP_NAME_ALIASES[cropName] ?? cropName
  const filename = CROPS_IMAGE_NAME_SUFFIX_1.has(canonicalName) ? `${canonicalName}1.png` : `${canonicalName}.png`
