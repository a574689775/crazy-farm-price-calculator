...
**目标产品**：《蛋仔派对》疯狂农场玩法  
**最后更新**：补充手机号登录/注册/找回、绑定手机号、邀请有礼与登录页交互；仅描述产品与交互，不含技术实现
...
| 模块           | 功能简述                                     | 是否需登录 |
|----------------|----------------------------------------------|------------|
| 登录/注册      | 邮箱或手机号 + 密码登录；邮箱验证码/手机号短信注册与找回密码；可选填邀请码 | 否（使用前需登录） |
...
访问 fknc.top
    ...
    → 已登录：进入「作物选择」页
        ...
        → 点击「个人中心」：打开侧边抽屉（昵称/头像、会员、计算历史、绑定手机号、邀请有礼、排行榜、联系我们、免责声明、用户隐私、更新日志、退出登录）
...
- **登录方式**：  
  - **邮箱登录**：邮箱 + 密码；支持「忘记密码？」（文本链接在密码框下方），通过邮箱验证码重置密码。  
  - **手机号登录**：手机号 + 密码（与邮箱登录交互一致）；支持「忘记密码？」（文本链接在密码框下方），通过手机号短信验证码重置密码。  
  - 登录页提供 Tab 切换「手机号登录」与「邮箱登录」（手机号推荐）；登录按钮下方居中展示「还没有账号？立即注册」，点击「立即注册」进入对应注册流程。  
- **注册方式**：  
  - **邮箱注册**：邮箱 + 邮箱验证码 + 密码 + 确认密码；可选填邀请码。  
  - **手机号注册**：手机号 + 短信验证码 + 密码 + 确认密码；可选填邀请码。  
  - 注册与找回密码时方使用验证码（邮件或短信），日常登录不发短信，成本可控。  
- **绑定手机号**：已用邮箱注册的老用户可在个人中心「绑定手机号」，通过短信验证码将手机号绑定到当前账号，绑定后可使用「手机号 + 原账号密码」登录。  
- **登录前**：需勾选同意《免责声明》和《用户隐私》；登录页不展示会员入口、使用教程等，仅保留备案区与登录相关表单。  
...
- **入口**：个人中心内「邀请有礼」（位于绑定手机号下方、排行榜上方）；首次进入作物选择页时自动弹出一次，关闭后仅能从个人中心再次打开。  
- **展示**：  
  - 用户专属邀请码，可一键复制。  
...
- **用户信息**：头像（多款可选）、昵称或手机号或邮箱、「会员用户/免费用户」标签；「修改个人信息」打开昵称与头像编辑弹窗。  
- **昵称与头像**：  
  - 昵称：最多 12 个字符、最多 6 个汉字，仅允许中文、字母、数字。  
  - 头像：在固定数量头像中选择其一保存。  
- **会员**：展示当前是否会员、到期日与剩余天数（若已开通）；按钮「开通会员」或「续费会员」打开会员弹窗。  
- **功能入口**：计算历史、绑定手机号（仅未绑定手机号时显示）、邀请有礼、排行榜。  
